  ...
  if (key == "fuel") {
    ...
    if (
      confirm(
        "Виплати за паливо " +
          fuelMounth +
          " місяць будуть видалені! Ви впевнені?"
      )
    ) {
      const myForm = new FormData(document.querySelector("#formPaid"));
      let response = await fetch("https://dov.pp.ua/miles/", {
...
async function checkDate2() {
  ...
  let responseMy = await fetch("https://dov.pp.ua/miles/", {
    method: "POST",
    body: formMyAdd,
  });
  ...
  if (arrMileage[0] > 0 || arrMileage[1] > 0) {
    getAlertCheck.innerHTML =
      "За обрану дату пробіг вже вказани! Оновити?<br><br>";
